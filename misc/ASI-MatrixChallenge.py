...
1) Perform DFS or BFS from top-left with target node being bottom-right
2) If bottom-right can be reached, return True
3) Otherwise, keep track of all nodes visited by search above. Save the set of all "0" nodes that are adjacent to any visited node, do this by iterating over all visited nodes
4) Next repeat DFS/BFS but this from bottom-right and top-left as target. Keep track of all visited nodes here too. Similar to above, save all "0" nodes that are adjacent to any visited node.
5) Count the intersection of the sets from Step 3 and 4. If it is zero, return "not possible", otherwise return the size of the set.
------------------------------------------------------------------------------------------------------------------------------------------------------------------
'''
